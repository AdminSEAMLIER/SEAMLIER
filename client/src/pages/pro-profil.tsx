...
function ProInfoSection() {
  const { data: proInfo } = useQuery<{ status?: string | null }>({
    queryKey: ["/api/professionnel/pro-info"],
  });

  return (
    <div className="rounded-xl border border-gray-100 shadow-sm bg-white p-6 space-y-3 mb-6">
      <h2 className="font-semibold text-gray-900 text-base">Informations professionnelles</h2>
      {proInfo?.status === "validated" ? (
        <p className="text-sm text-green-600 font-medium">✓ Dossier validé</p>
      ) : (
        <p className="text-sm text-amber-600">En cours de vérification par l'équipe SEAMLiER.</p>
      )}
      <p className="text-xs text-gray-400">
        SIRET, IBAN et RC Pro sont gérés par l'équipe SEAMLiER. Envoyez vos justificatifs via la messagerie.
      </p>
    </div>
  );
}
